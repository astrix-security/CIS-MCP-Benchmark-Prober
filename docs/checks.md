...
- **A bearer token is released only over https, and only to the audited
  ...
  own name and not its neighbours'. Check 3.5 records every candidate the gate
  refused, with the reason.

Check 3.6 registers real OAuth clients at the audited authorization server. Those
...
Check 3.5's `3.5b` leg spends the cached refresh token, so it runs as the last
...
### 3.1 Remote servers require authentication and issue short-lived tokens
...
- `3.1a` — the unauthenticated `initialize` round-trip, recorded once before any
...
- `3.1b` — the issued lifetime against a 3600-second baseline, read from the
  ...
  one.
- `3.1c` and `3.1d` — the authorization server's advertised
...
**Note on `3.1a`.** Endpoint detection accepts a candidate only on a 200 or a
...
readable lifetime, so `3.1b` records `UNKNOWN` and the check reports `UNKNOWN`
rather than passing on an unread value.

### 3.2 stdio credentials are sourced from the environment or a credential store
...
### 3.3 Per-tool authorization policies are enforced
...
### 3.4 The issued token's audience is confined to the audited resource
...
### 3.5 The issued access token is bound to the audience that requested it
...
- `3.5a` — the `aud` claim names the canonical resource URI.
- `3.5c` — a downstream identity endpoint must refuse the token. Candidate hosts
  ...
  credential onward.
- `3.5b` — the refresh grant asks the authorization server for a token bound to
...
**Reduction.** `3.5c`'s candidates are guesses, not downstream APIs discovered
...
two-label endpoint host yields no derived candidate at all. `3.5b` cannot confirm
...
### 3.6 An OAuth proxy validates redirect_uri, callback state and consent
...
- `3.6-pre` — whether the recommendation applies. A server whose authorization
...
- `3.6a` — an authorize request varying only `redirect_uri`, to a host under
  ...
  fails.
- `3.6b` — the callback must refuse a fabricated code carrying a state that was
  never issued. A nonsense path on the same origin is fetched as a control.
- `3.6c` — a client id the authorization server has never seen must meet a
  consent step, rather than being handed straight to the upstream authorization
  server on the shared client id.

**Attribution rules.** `3.6b`'s control is a parameter of the decision, not a
...
because a gateway serves those too. On `3.6a`, a refusal of the varied value
...
`3.6b` sends a fabricated code, so the invalid code alone may explain a refusal
that the missing state would also have earned.

### 3.7 Server-provided tool annotations are not relied upon for gating
...
### 3.8 Granted OAuth scopes are minimized
...
- `3.8a` — the granted scopes are readable at all.
- `3.8b` — no granted scope is a wildcard.
- `3.8c` — the granted set has not grown since the recorded baseline.
- `3.8d` — the advertised `scopes_supported` carries no wildcard and no
  admin-tier scope.
- `3.8e` — a tool the operator names as outside the grant is refused rather than
...
- `3.8f` — at least one of three scope-discovery sources carries a value: the
  challenge `scope` parameter, the protected-resource `scopes_supported`, or the
  authorization server's own `scopes_supported`.

**Attribution rules.** `3.8f` fails only when all three sources are absent *and*
...
disagreement is reported, not failed. On `3.8e`, a refusal counts only when it
...
a client, so a documented exception for a flagged scope would change `3.8b` and
`3.8d`. A server offering no OAuth at all reports `N/A`: it has no granted or
advertised scope to minimize.

**A comparison needs two observed sides.** Leg `3.8c` reports `UNKNOWN` when either
...
grant has stated the grant, so legs `3.8a` and `3.8c` both treat it as observed.

### 3.9 Downstream service identities are not shared across tools or servers
...
### 3.10 OAuth discovery metadata is served over TLS and validated
...
- `3.10a` — the MCP endpoint is served over TLS. This is a precondition, not a
  graded property: check 2.2 owns the transport verdict, and grading it again
  would fail 3.10 for something already reported. A non-TLS endpoint leaves no
  chain below it to assess, so the check reports `UNKNOWN`.
- `3.10b` — the document a client selects agrees with the document the 401 challenge
...
- `3.10c` — every discovery path that answered was https.
...
- `3.10g` — each advertised authorization server publishes an `issuer` that
...
**Wider comparison on `3.10d`.** The probe applies the SDK's parent-prefix rule —
same origin, canonical path at or below the advertised path — rather than an exact
string match, because that is what a conforming client applies. Every `3.10d`
...
Where the guard refused a URL the server supplied, `3.10c` records `UNKNOWN`: the
URL was never fetched, so its scheme was never seen.

Leg `3.10f` follows the same two-observed-sides rule as `3.8c`. A document that
answered and advertises no authorization server is an observed absence, so the leg
decides against the record rather than reporting `UNKNOWN` — the same reading `3.8f`
...
| 3.1 | Authentication required, tokens short-lived | **FAIL** | **FAIL** | PASS |
| 3.2 | stdio credentials from environment or store | N/A | N/A | N/A |
| 3.3 | Per-tool authorization enforced | N/A | N/A | N/A |
| 3.4 | Token audience confined to audited resource | N/A | UNKNOWN | UNKNOWN |
| 3.5 | Token bound to the audience that requested it | N/A | UNKNOWN | UNKNOWN |
| 3.6 | OAuth proxy validates redirect_uri, state, consent | N/A | N/A | N/A |
| 3.7 | Tool annotations not relied on for gating | N/A | N/A | N/A |
| 3.8 | Granted scopes minimized | N/A | UNKNOWN | UNKNOWN |
| 3.9 | Downstream identities not shared | N/A | N/A | N/A |
| 3.10 | Discovery metadata over TLS and validated | N/A | PASS | PASS |
...
- **3.1 — 1/3 pass, and the two failures fail for opposite reasons.** DeepWiki
  requires no authentication, so an unauthenticated request reached a response and
  leg `3.1a` fails. Linear refuses correctly and names `resource_metadata`, then
  fails leg `3.1b`: its issued lifetime is 86100 seconds against the 3600-second
...
- **3.4 and 3.5 are `UNKNOWN` on Linear because its access token is opaque.** No
  `aud` claim is readable, so neither audience leg can be observed. Leg `3.5c`
  ...
  `3.5b` reached the wire and got the conforming answer: Linear's authorization
...
- **3.8 is `UNKNOWN` on Linear because leg `3.8e` has no operator input.** No tool
...
- **3.10 passes on Linear.** Two discovery paths answered and serve the same
...
- **3.4 and 3.5 are `UNKNOWN` on Stripe too, and its access token is opaque as
  well.** Leg `3.5c` produced the one live pass of that leg so far:
  ...
  answered the same either way and decided nothing. Leg `3.5b` is `UNKNOWN` for a
...
- **3.8 is `UNKNOWN` on Stripe on two legs.** Granted scope is `mcp`, no wildcard,
  and within the recorded baseline. `3.8d` is undecided because the
  protected-resource document advertises no `scopes_supported` at all, and `3.8e`
  because no operator input names a tool outside the grant.
  `3.8f` passed on the third discovery source alone: the challenge carried no
...
- **3.10 passes on Stripe, and it took two runs.** Six legs passed on the first
  ...
  a published `issuer` that string-equals it. Leg `3.10f` was undecided, because no
  ...
  compared against it and passed. `3.10f` is the one leg here that reports on drift
  rather than on a property of a single run, so it needs two runs by construction.
- **3.6 is `N/A` on all three targets, and each reaches that verdict differently.**
...
Ten checks reporting verdicts do not mean the coverage is complete. Four gaps
remain, and each names what closing it needs.

1. **Check 3.6's three test legs — `3.6a`, `3.6b` and `3.6c` — have no live
...
2. **Leg 3.8e is unverified on every target**, because no operator input file
...
3. **Leg 3.5b's rotation path is untested.** No authorization server has minted a
   ...
   first time.
4. **Sentry has no Section 3 column at all.** See below.

Two legs need two runs by construction, because both report on drift rather than on
a property of one run: `3.8c` compares the granted scope set against the baseline,
and `3.10f` compares the advertised authorization server list. Both are decided on
every target in the table. On a server that rotates refresh tokens, that pair of
runs used to cost two interactive logins, because `3.5b` spent the cached token.
