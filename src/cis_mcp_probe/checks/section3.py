...
def _static_client_id_verdict(first: str | None, second: str | None) -> str:
    """Whether two registrations share one onward client id, from what each carried.

    The condition is a client id *shared* across registrations, so one repeated id
    establishes it and two different ids rule it out. A redirect that names no
    client at all decides neither: ``"blind"`` is undecided, not ruled out.
    """
    if not first or not second:
        return "blind"
    return "applies" if first == second else "vacuous"
...
class ConfusedDeputySafeguards(Check):
    ...
    async def run(self, ctx: ProbeContext) -> CheckResult:
        endpoint = ctx.endpoint_url
        if not endpoint:
            return self._error("no live session to test against." + PROXY_REDUCTION)
        ...
        first_location = first[1].get("location", "")
        second_location = second[1].get("location", "")
        first_downstream = _downstream_client_id(first_location)
        second_downstream = _downstream_client_id(second_location)
        condition = _static_client_id_verdict(first_downstream, second_downstream)
        if condition == "blind":
            return self._unknown(
                f"3.3.5-pre: registered {first_id} and {second_id}, and their "
                f"authorize responses (HTTP {first[0]}, {second[0]}) redirected to "
                f"{first_location!r} and {second_location!r}. An onward client id "
                "was not read from one or both, so whether this server fronts a "
                "separate authorization server on one shared static client id is "
                "undecided. "
                "This run neither grades the safeguards nor rules the "
                "recommendation out." + PROXY_REDUCTION,
                legs={"3.3.5-pre": "unknown"},
                registrations=registrations,
