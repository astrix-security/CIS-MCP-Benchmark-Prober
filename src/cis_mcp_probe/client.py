...
async def _resolve_authorization_servers(ctx: ProbeContext) -> None:
    ...
    advertised = prm.get("authorization_servers") if isinstance(prm, dict) else None
    if not isinstance(advertised, list):
        return

    entries = [e for e in advertised if isinstance(e, str) and e.strip()]
    ignored = len(advertised) - len(entries)
    if ignored:
        ctx.errors.append(
...
def _reset_session_state(ctx: ProbeContext) -> None:
    ...
    check 3.8 with no scope-discovery input at all, and check 3.10 with nothing to
...
if __name__ == "__main__":
    from mcp.client.auth.utils import (
        build_oauth_authorization_server_metadata_discovery_urls as _as_urls,
        build_protected_resource_metadata_discovery_urls as _prm_urls,
    )

    # test-type: ground-truth-vendor | source: mcp.client.auth.utils
    # captured-from: mcp 1.28.1 | last-revalidated: 2026-08-27
    # Check 3.10 grades servers against exactly the URL set our own OAuth client
    # would try, so the vendor's lists are pinned rather than restated.
    # An issuer with a path component yields THREE forms, not four.
    assert _as_urls("https://as.example.com/mcp", "https://as.example.com/mcp") == [
    ...
    assert _prm_urls(None, "https://mcp.linear.app/mcp") == [
        "https://mcp.linear.app/.well-known/oauth-protected-resource/mcp",
        "https://mcp.linear.app/.well-known/oauth-protected-resource",
    ]
    # Endpoint discovery. A bare domain yields the default paths in order. An
    # explicit path is taken as given and discovery is skipped, which is how a server
    # whose endpoint sits outside the default list is reached at all.
    _host, _base, _cands = normalize("mcp.example.com")
    assert _host == "mcp.example.com" and _base == "https://mcp.example.com"
    assert _cands == [
        "https://mcp.example.com/mcp",
        "https://mcp.example.com/",
    ], _cands
    assert normalize("https://mcp.example.com/v1/sse")[2] == [
        "https://mcp.example.com/v1/sse"
    ]

    print("client: discovery-url self-checks passed")
