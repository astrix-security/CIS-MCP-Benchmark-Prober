...
def _seed_refresh_path(
    provider: OAuthClientProvider,
    storage: FileTokenStorage,
    auth_server_metadata: dict | None,
) -> None:
    """Give the provider what its own refresh grant needs before the first request.

    Two things are missing on a cold start, and each one alone is enough to stop
    the refresh:

    * No deadline. The provider computes one when it receives a token, never when
      it loads one, and it reads a missing deadline as "still valid". So it sends a
      stale token, takes the 401, and starts a full authorization flow instead.
    * No authorization-server metadata. Its refresh request falls back to
      ``<server base>/token`` when it holds none, which is a 404 on any server
      whose token endpoint lives elsewhere. Metadata is normally fetched on the
      401 path, which is *after* the refresh would have run.

    Both are already known here: the deadline from the stored record, and the
    metadata from the discovery this run performed before the session. Either one
    being absent leaves that half alone, which is the behaviour without this.
    """
    ...
    if auth_server_metadata:
        try:
            ...
            # A document that does not validate leaves the fallback URL in place.
            # The refresh then fails as it does today and the flow continues.
            pass
...
def _make_oauth_provider(
    ...
    auth_server_metadata: dict | None = None,
) -> OAuthClientProvider:
    ...
    _seed_refresh_path(provider, storage, auth_server_metadata)
    return provider
...
async def _session_attempt(
    ...
    provider = _make_oauth_provider(
        endpoint, storage, callback, ctx.auth_server_metadata
    )
